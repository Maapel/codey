...
export async function evalsEnvHandler(options: EvalsEnvOptions): Promise<void> {
	...
	switch (options.action) {
		case "create":
			...
			console.log(chalk.green("The Codey extension should now detect this file and enter test mode."))
		...
		case "remove":
			...
			console.log(chalk.green("The Codey extension should now exit test mode."))
			console.log(chalk.yellow("Note: You may need to reload VSCode for the changes to take effect."))
			break
		...
		case "check":
			...
			if (exists) {
				console.log(chalk.green("The Codey extension should be in test mode."))
			} else {
				console.log(chalk.yellow("The Codey extension should not be in test mode."))
			}
			break
