...
/**
 * Interactive Playwright launcher for the Codey VS Code extension.
 *
 * Overview:
 *  - Starts the mock Codey API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the Codey extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the Codey sidebar.
 ...
 *
 *   3. VS Code will launch with the Codey extension loaded and gRPC recording enabled.
 *
...
async function main() {
	...
	console.log("VSCode with Codey extension is now running!")
