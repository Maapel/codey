...
export async function* runClaudeCode(options: ClaudeCodeOptions): AsyncGenerator<ClaudeCodeMessage | string> {
	...
	const tempFilePath = path.join(os.tmpdir(), `codey-system-prompt-${uniqueId}.txt`)
...
`,
			...
			if (err.message.includes("ENAMETOOLONG")) {
				throw new Error(
					`Executing Claude Code failed due to a long system prompt. Windows has a limit of 8191 characters, which makes the integration with Codey not work properly.
Please check our docs on how to integrate Claude Code with Codey on Windows: https://docs.codey.bot/provider-config/claude-code#windows-setup.
Anthropic is aware of this issue and is considering a fix: https://github.com/anthropics/claude-code/issues/3411.
`,
					{ cause: err },
				)
...
function runProcess(
	...
	const args = [
		...
		"--disallowedTools",
		claudeCodeTools,
		// Codey will handle recursive calls
		"--max-turns",
		"1",
		"--model",
		modelId,
