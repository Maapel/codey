...
log("Running standalone codey", ExtensionRegistryInfo.version)
...
export function initializeContext(clineDir?: string) {
	const CLINE_DIR = clineDir || process.env.CLINE_DIR || `${os.homedir()}/.codey`
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in codey.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in codey.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in codey.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in codey.
