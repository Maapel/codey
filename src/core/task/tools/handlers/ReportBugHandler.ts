	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enabled && config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Codey wants to create a github issue...",
				message: `Codey is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("codey-version", clineVersion)
				...
				await createAndOpenGitHubIssue("codey", "codey", "bug_report.yml", params)
			} catch (error) {
